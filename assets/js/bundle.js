...
const state={periods:[],period:null,month:null,charts:{},teamConfig:{employees:[]},settings:{},security:{protectedPages:['manager','settings'],sessionKey:'powerAnalyticsManagerAuth'},pendingProtectedPage:null};
...
function managerIsAuthenticated(){
  return sessionStorage.getItem(state.security.sessionKey||'powerAnalyticsManagerAuth')==='true';
}
function updateManagerSessionUI(){
  const button=$('managerSessionBtn');
  const logged=managerIsAuthenticated();
  if(button){button.textContent=logged?'🔓 Sair do gestor':'🔒 Área do gestor';button.classList.toggle('authenticated',logged)}
  document.querySelectorAll('.protected-nav').forEach(item=>{const page=item.dataset.page;item.classList.toggle('unlocked',logged);item.textContent=`${logged?'🔓':'🔒'} ${page==='manager'?'Central do Gestor':'Gestão da Equipe'}`})
}
function openManagerAuth(page=null){state.pendingProtectedPage=page;$('managerAuthError').textContent='';$('managerPassword').value='';const overlay=$('managerAuthOverlay');overlay.classList.add('open');overlay.setAttribute('aria-hidden','false');setTimeout(()=>$('managerPassword').focus(),60)}
function closeManagerAuth(){const overlay=$('managerAuthOverlay');overlay.classList.remove('open');overlay.setAttribute('aria-hidden','true');state.pendingProtectedPage=null}
...
async function authenticateManager(password){const typedHash=await sha256(password);if(typedHash!==state.security.managerPasswordHash)return false;sessionStorage.setItem(state.security.sessionKey||'powerAnalyticsManagerAuth','true');updateManagerSessionUI();return true}
function managerLogout(){sessionStorage.removeItem(state.security.sessionKey||'powerAnalyticsManagerAuth');updateManagerSessionUI();const active=document.querySelector('.nav-btn.active')?.dataset.page;if((state.security.protectedPages||[]).includes(active))document.querySelector('.nav-btn[data-page="dashboard"]')?.click();showStatus('<b>Sessão do gestor encerrada.</b> As áreas administrativas foram bloqueadas novamente.','success');setTimeout(clearStatus,2800)}
function protectedNavigationAllowed(page){return !(state.security.protectedPages||[]).includes(page)||managerIsAuthenticated()}
function bindManagerSecurity(){
  document.querySelectorAll('.nav-btn').forEach(button=>button.addEventListener('click',event=>{const page=button.dataset.page;if(!protectedNavigationAllowed(page)){event.preventDefault();event.stopImmediatePropagation();openManagerAuth(page)}},true));
  $('managerAuthClose').addEventListener('click',closeManagerAuth);$('managerAuthCancel').addEventListener('click',closeManagerAuth);$('managerAuthOverlay').addEventListener('click',event=>{if(event.target===event.currentTarget)closeManagerAuth()});
  $('toggleManagerPassword').addEventListener('click',()=>{const field=$('managerPassword');field.type=field.type==='password'?'text':'password';$('toggleManagerPassword').textContent=field.type==='password'?'👁':'🙈'});
  $('managerAuthForm').addEventListener('submit',async event=>{event.preventDefault();const submit=event.currentTarget.querySelector('button[type="submit"]');submit.disabled=true;submit.textContent='Validando...';try{const success=await authenticateManager($('managerPassword').value);if(!success){$('managerAuthError').textContent='Senha incorreta. Tente novamente.';$('managerPassword').select();return}const target=state.pendingProtectedPage;closeManagerAuth();if(target)document.querySelector(`.nav-btn[data-page="${target}"]`)?.click()}finally{submit.disabled=false;submit.textContent='Entrar'}});
  $('managerSessionBtn').addEventListener('click',()=>managerIsAuthenticated()?managerLogout():openManagerAuth());
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('managerAuthOverlay').classList.contains('open'))closeManagerAuth()});
  updateManagerSessionUI();
}
...
async function loadDefault(){clearStatus();try{
  const [teamResponse,settingsResponse,securityResponse]=await Promise.all([
    fetch(`config/team.json?v=${Date.now()}`,{cache:'no-store'}),
    fetch(`config/settings.json?v=${Date.now()}`,{cache:'no-store'}),
    fetch(`config/security.json?v=${Date.now()}`,{cache:'no-store'})
  ]);
  ...
  if(securityResponse.ok)state.security={...state.security,...await securityResponse.json()};
  loadSavedTeam();
  try{
...
document.addEventListener('DOMContentLoaded',()=>{bind();bindManagerSecurity();loadDefault()});
})();
