...
function loadSavedTeam(){
  try{
    ...
    if(saved)state.teamConfig=JSON.parse(saved);
...
function renderAll(){try{renderKpis();renderInsights();renderDashboardCharts();renderTeam();renderHall();renderBox();renderBonus();renderVoice();renderHistory();renderManager();renderTeamAdmin();renderAudit();clearStatus()}catch(err){console.error(err);showStatus(`<b>Erro ao montar o dashboard.</b> ${err.message}`)}}
