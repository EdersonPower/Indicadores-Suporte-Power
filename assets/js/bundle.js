...
function renderEmployeeOfMonth(){const w=employeeOfMonthData();if(!w){$('employeeOfMonth').innerHTML='<div class="empty">Sem dados.</div>';return}$('employeeOfMonth').innerHTML=`<div class="employee-of-month-glow"></div><span class="eyebrow">🏆 Colaborador do período</span><div class="employee-feature">${avatar(w.name,'executive-avatar')}<div><h2>${w.name}</h2><p>Desempenho combinado de qualidade, volume, avaliações e Caixinha</p></div></div><div class="employee-feature-stats"><div><span>Média</span><b>${fmt.dec(w.final)}</b></div><div><span>Atendimentos</span><b>${fmt.int(w.att)}</b></div><div><span>Taxa</span><b>${fmt.pct(w.rate)}</b></div><div><span>Caixinha</span><b>${fmt.dec(w.box||0,1)}</b></div></div>`}
